...
const SubscriptionAction = (props: Props) => {
  ...
  return (
    <div className='flex flex-col items-center w-1/2 p-4 mx-auto mt-4 rounded-md bg-secondary'>
        {data?.user.credits} / 10 Free Generaion
        <Progress className='mt-2' 
        value={data?.user.credits ? (
          (data?.user.credits) / 10 * 100
        ) : 0} />
        <Button disabled={loading} onClick={handleSubscribe} className="mt-3 font-bold text-white transition bg-gradient-to-tr from-green-500 to-blue-500 hover:from-green-500 hover:to-blue-600 ">
          Upgrade
